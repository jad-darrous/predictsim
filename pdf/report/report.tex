...
\begin{abstract}
  ...
  Predictive techniques are investigated in order to infer the run time of jobs from both their description and system history. A Machine Learning technique (Random Forests) is applied and compared against the state of the art.
  Experiments emphasize that the predictions obtained with this approach have correct properties with respect to relevant metrics, and outperform pre-existing approaches.
\end{abstract}

\section{Introduction}
High Performance Computing (HPC) systems are more and more complex.
%TODO
There is a need for studies and tools for efficient use, at the frontier of two different fields: scheduling and allocation of jobs, and system and middleware. We outline two of the main difficulties that Resource Management Software (RMS) in this field have to face.

First, the ephemeral nature and broad range of existing architectures of those systems make it difficult to develop and apply of theoretical results.
