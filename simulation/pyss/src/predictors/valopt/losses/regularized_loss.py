...
class RegularizedLoss(object):
    ...
    def loss(self,x,y):
        ...
        return self.orig_loss.loss(x,y)+self.alpha*self.regularizer.norm(self.model.get_param_vector())

    def d_loss_directional(self,x,y,i):
        """Return the derivative of the loss with respect to the i-th entry of the parameter vector of the model"""
        return self.orig_loss.d_loss_directional(x,y,i)+self.alpha*self.regularizer.d_norm_directional(self.model.get_param_vector(),i)

    def grad_loss(self,x,y):
        """Gradient of the loss of the model on example (x,y) """
        return self.orig_loss.grad_loss(x,y) + self.alpha*self.regularizer.grad_norm(self.model.get_param_vector())
